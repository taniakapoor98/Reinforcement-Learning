...
class StreetFighterEnv(gym.Env):
    def __init__(self,game=None):
        ...
        self.action_space = spaces.Discrete(5)  # left right attacks  # 5  * 4 change to multibinary later
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([
                ...
                3,  # Player 1 attack states: light, medium, heavy (binary)
                3 # Player 2 attack states: light, medium, heavy (binary)
            ]),
            dtype=np.float32
        )
    ...
    def _compute_reward(self):
        ...
        if p2_health < self.prev_p2_health:
            reward += 1
        if p1_health < self.prev_p1_health:
            reward -= 1
    ...
    def _check_done(self):
        ...
        elif self.game.getTimeLeft() <= 0:
            if p1_health > p2_health:
                result = "P1 Won (Time)"  # Player 1 won on health points when time ran out
            elif p2_health > p1_health:
                result = "P1 Lost (Time)"  # Player 1 lost on health points when time ran out
            else:
                result = "Draw"  # Draw because health is equal or time ran out

        # Check if P1 won via game win counter
        elif p1_wins > 0:
            result = "P1 Won (Game)"  # Player 1 won via game win counter

        # Check if P2 won via game win counter
        elif p2_wins > 0:
            result = "P1 Lost (Game)"  # Player 1 lost via game win counter
