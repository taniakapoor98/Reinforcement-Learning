...
register(
    ...
    entry_point='StreetFighterEnv:StreetFighterEnv',  # Adjust the entry point as necessary
)
class KeyEvent:
    VK_ENTER = 10
    VK_UP = 38
...
num_timesteps = 10000
for _ in range(num_timesteps // 100):  # Loop to run episodes
    obs = env.reset()
    total_reward = 0
    done = False
    while not done:
        action, _ = model.predict(obs)  # Get action from the model
        obs, reward, done,info, _ = env.step(action)  # Step through the environment
        total_reward += reward
        model.learn(total_timesteps=1)  # Update the model with each action (optional)

    training_rewards.append(total_reward)  # Store total reward for the episode
